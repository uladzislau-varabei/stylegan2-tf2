...
from utils import NCHW_FORMAT, NHWC_FORMAT, validate_data_format
from tf_utils import DEFAULT_DATA_FORMAT


DEFAULT_IMPL = cfg.DEFAULT_IMPL


#----------------------------------------------------------------------------

def upfirdn_2d(x, k, upx=1, upy=1, downx=1, downy=1, padx0=0, padx1=0, pady0=0, pady1=0, impl=DEFAULT_IMPL):
    ...
    impl_dict = {
        'ref':  _upfirdn_2d_ref,
        'cuda': None,
        'custom_grad': _upfirdn_2d_ref_custom_grad
    }
...
def _upfirdn_op(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    kernelH, kernelW = k.shape
    inH = x.shape[1]
    inW = x.shape[2]
    minorDim = x.shape[3]
...
def _upfirdn_2d_ref_custom_grad(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    """Potentially fast implementation of `upfirdn_2d()` with custom gradient based on CUDA version."""
    ...
    kc = np.asarray(k, dtype=x.dtype.name)
    gkc = np.asarray(k[::-1, ::-1], dtype=x.dtype.name)
    ...
    @tf.custom_gradient
    def func(x):
        y = _upfirdn_op(x=x, k=kc, upx=int(upx), upy=int(upy), downx=int(downx), downy=int(downy), padx0=int(padx0), padx1=int(padx1), pady0=int(pady0), pady1=int(pady1))
        y.set_shape([majorDim, outH, outW, minorDim])
        @tf.custom_gradient
        def grad(dy):
            dx = _upfirdn_op(x=dy, k=gkc, upx=int(downx), upy=int(downy), downx=int(upx), downy=int(upy), padx0=int(gpadx0), padx1=int(gpadx1), pady0=int(gpady0), pady1=int(gpady1))
...
def filter_2d(x, k, gain=1, padding=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
...
def upsample_2d(x, k=None, factor=2, gain=1, padding=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
...
def downsample_2d(x, k=None, factor=2, gain=1, padding=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
...
def upsample_conv_2d(x, w, k=None, factor=2, gain=1, padding=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
...
def conv_downsample_2d(x, w, k=None, factor=2, gain=1, padding=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
...
def _simple_upfirdn_2d(x, k, up=1, down=1, pad0=0, pad1=0, data_format=DEFAULT_DATA_FORMAT, impl=DEFAULT_IMPL):
